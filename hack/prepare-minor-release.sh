...
sed -i "" "1r hack/changelog-template.txt" $DOCS_TO_UPDATE_FOLDER/content/releases.md 
sed -i "" -e "s/%%TITLE%%/## NGINX Ingress Controller $ic_version/g" -e "s/%%IC_VERSION%%/$ic_version/g" -e "s/%%HELM_CHART_VERSION%%/$helm_chart_version/g" $DOCS_TO_UPDATE_FOLDER/content/releases.md
...
sed -i "" "s/$prev_ic_version/$ic_version/g" $DOCS_TO_UPDATE_FOLDER/content/technical-specifications.md 

# update IC version in the building ingress controller doc
sed -i "" "s/$prev_ic_version/$ic_version/g" $DOCS_TO_UPDATE_FOLDER/content/installation/building-ingress-controller-image.md

# update IC version in the helm doc  
sed -i "" "s/$prev_ic_version/$ic_version/g" $DOCS_TO_UPDATE_FOLDER/content/installation/installation-with-helm.md
