...
1. Build the image:
    * For **NGINX**:
      ...
      As a result, the image **myregistry.example.com/nginx-ingress:1.13.0** is built. Note that the tag `1.13.0` comes from the `VERSION` variable, defined in the Makefile.
    ...
    * For **NGINX Plus**, first, make sure that the certificate (`nginx-repo.crt`) and the key (`nginx-repo.key`) of your license are located in the root of the project:
      ...
      As a result, the image **myregistry.example.com/nginx-plus-ingress:1.13.0** is built. Note that the tag `1.13.0` comes from the `VERSION` variable, defined in the Makefile.
